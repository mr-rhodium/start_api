from app.clients.database.vehicles_database_client import (
    insert_new_vehicle,
    get_vehicle_by_id as get_vehicle_by_id_from_database,
)
...
def add_new_vehicle(
    ...
    new_vehicle: StarWarsVehicleModel = insert_new_vehicle(db, swapi_vehicle)

    return StarWarsVehicleRead.model_validate(new_vehicle)


def get_vehicle_by_id(vehicle_id: int, db: Session) -> StarWarsVehicleRead:
    vehicle = get_vehicle_by_id_from_database(db, vehicle_id)
